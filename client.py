...
def redrawWindow(win, player1, player2, round_status, p1_win, p2_win, draw_pos, game_over1):
    ...
    draw_text(f"Rev: {player1.rev}", extra_coolfont, LIGHT_RED, 25, 225)

    draw_text(f"Current: {current_player_turn}", extra_coolfont, LIGHT_RED, 1225, 25)

    if num_pauses == 4:
        if p1.rev == 1:
            draw_text(f"You: {str(player1.disc_data[0])}", extra_coolfont, LIGHT_RED, 325, 200)
        elif p1.rev == 2:
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])}", extra_coolfont, LIGHT_RED, 325, 200)
        elif p1.rev == 3:
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])}", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data[0])}", extra_coolfont, LIGHT_RED, 325, 300)
        elif p1.rev == 4:
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])}", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data[0])+ str(player2.disc_data[1])}", extra_coolfont, LIGHT_RED, 325, 300)
        else:
            draw_text(f"You: {str(player1.disc_data)}+' '+ {str(p1_win)} ", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data)}+' '+ {str(p2_win)} ", extra_coolfont, LIGHT_RED, 325, 300)
    elif num_pauses == 6:
        if p1.rev == 1:
            draw_text(f"You: {str(player1.disc_data[0])}", extra_coolfont, LIGHT_RED, 325, 200)
        elif p1.rev == 2:
            ...
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])+ str(player1.disc_data[2])}", extra_coolfont, LIGHT_RED, 325, 200)
        elif p1.rev == 4:
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])+ str(player1.disc_data[2])}", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data[0])}", extra_coolfont, LIGHT_RED, 325, 300)
        elif p1.rev == 5:
            ...
            draw_text(f"You: {str(player1.disc_data[0])+ str(player1.disc_data[1])+ str(player1.disc_data[2])}", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data[0])+ str(player2.disc_data[1])+ str(player2.disc_data[1])}", extra_coolfont, LIGHT_RED, 325, 300)
        else:
            draw_text(f"You: {str(player1.disc_data)}+' '+ {str(p1_win)} ", extra_coolfont, LIGHT_RED, 325, 200)
            draw_text(f"Them: {str(player2.disc_data)}+' '+ {str(p2_win)} ", extra_coolfont, LIGHT_RED, 325, 300)

    draw_text(f"You: {str(player1.disc_data)}+' '+ {str(p1_win)} ", extra_coolfont, LIGHT_RED, 325, 400)
    draw_text(f"Them: {str(player2.disc_data)}+' '+ {str(p2_win)} ", extra_coolfont, LIGHT_RED, 325, 500)
    ...
    draw_text(f"p2 amount bet: ${player2.player_bet}", extra_coolfont, LIGHT_RED, 1025, 725)

    draw_text(f"pot: ${pot}", extra_coolfont, LIGHT_RED, 425, 25)
...
def game__over(win, p1, p2, p1_win, p2_win, draw_pos, p1_res, p2_res):
    ...
    draw_text(f"p1 res: ${p1_res}", extra_coolfont, LIGHT_RED, 25, 25)
    draw_text(f"p2 res: ${p2_res}", extra_coolfont, LIGHT_RED, 25, 125)
    if p1_res > p2_res:
        draw_text(f"winner: u", extra_coolfont, LIGHT_RED, 25, 225)
    elif p2_res > p1_res:
        draw_text(f"winner: no u", extra_coolfont, LIGHT_RED, 25, 225)
    else:
        draw_text(f"winner: tie", extra_coolfont, LIGHT_RED, 25, 225)
    draw_text(f"chance to win: ${p1_win}", extra_coolfont, LIGHT_RED, 25, 325)
    pygame.display.update()
